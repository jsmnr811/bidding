        ...
        window.mapSearch = function(provinceGeo, temporaryGeo) {
            return {
                ...
                initMap() {
                    ...
                    // Force marker reposition on zoom end
                    // this.map.on('zoomend', () => {
                    //     this.markersProvince.forEach(m => m.update());
                    //     this.markersTemporary.forEach(m => m.update());
                    // });
                ...
                addMarkers(data, isTemporary) {
                    ...
                    points.forEach(entry => {
                        ...
                        // Generate popup HTML without inline onclick
                        const popupContent = this.getPopupHTML(entry, isTemporary);

                        marker.bindPopup(popupContent);

                        // Attach popup event handlers on popupopen (only once)
                        ...
                        marker.on('popupclose', e => {
                            ...
                            // Remove or nullify popup or marker references you may hold
                ...
                getPopupHTML(entry, isTemporary = false) {
                    ...
                    // Button has class for JS binding, no inline onclick
                    return `
        ...
        <div class="d-grid mt-2">
            <button id="add-commodity-btn" class="btn btn-sm btn-success">Add</button>
                ...
                setupPopupHandlers(popupEl) {
                    ...
                    // Disable map scroll on dropdown focus
                    $(popupEl).on('mouseenter', '.select2-container', () => {
                        this.map.scrollWheelZoom.disable();
                    });
                    $(popupEl).on('mouseleave', '.select2-container', () => {
                    ...
                    if (addBtn) {
                        addBtn.addEventListener('click', () => {
                            ...
                            this.$wire.set('lat', markerLatLng.lat);
                            this.$wire.set('lon', markerLatLng.lng);
                    ...
                    // Update Livewire lat/lon whenever popup opens, just for safety
                    const markerLatLng = this.marker.getLatLng();
                    this.$wire.set('lat', markerLatLng.lat);
                    this.$wire.set('lon', markerLatLng.lng);
                },
